...
plt.show()
'''
plt.plot(data.index, real_EPP_values, linestyle='-', color='#4169E1', label='Real EPP')  # 深蓝色
plt.plot(data.index, data['Generated_EPP_Normalized'], linestyle='-', color='#FF0000', label='Generated EPP (Normalized)')  # 深红色
plt.title('Real EPP vs. Generated EPP (Normalized) with Euclidean Distance')
plt.xlabel('A-BEL model')
plt.ylabel('EPP Value')
plt.legend()
plt.grid(True)
plt.show()
